...
class IMS:
    def __init__(self, root):
        ...
        title = Label(self.root, text="Papri Filling Station", image=self.icon_title, compound=LEFT,
                      font=("arial", 40, "bold"), bg="#010c48", fg="white", anchor="w", padx=20)
        title.place(x=0,y=0,relwidth=1)

        # ====btn_logout====
        
        btn_logout = Button(self.root, text="Logout", command=self.logout, font=("arial", 15, "bold"),
                            bg="yellow", cursor="hand2").place(x=1150, y=10, width=150, height=50)

        # ====clock====
        
        self.lbl_clock = Label(self.root,
                               text="Welcome to Papri Filling Station\t\t Date: DD-MM-YYYY\t\t Time: HH:MM:SS",
                               font=("arial", 15), bg="#4d636d", fg="white")
        ...
        lbl_menu = Label(LeftMenu, text="Menu", font=("arial", 12), bg="#009688").pack(side=TOP, fill=X)

        # ====Menu Buttons===
        
        btn_employee = Button(LeftMenu, text="Employee", command=self.employee, image=self.icon_side, compound=LEFT,
                              padx=5, anchor="w", font=("arial", 20, "bold"), bg="white", bd=3,
        ...
        btn_sales = Button(LeftMenu, text="Sales", command=self.sales, image=self.icon_side, compound=LEFT, padx=5,
                           anchor="w", font=("arial", 20, "bold"), bg="white", bd=3, cursor="hand2").pack(
            side=TOP, fill=X)
        btn_exit = Button(LeftMenu, text="Exit", image=self.icon_side, compound=LEFT, padx=5, anchor="w",
                          font=("arial", 20, "bold"), bg="white", bd=3, cursor="hand2").pack(side=TOP, fill=X)
        ...
        lbl_footer = Label(self.root,
                           text="Filling Station Management System | Devloped by Sarwar Hossain. For any query, Please contact at +8801719544363",
                           font=("arial", 12), bg="#4d636d", fg="white").pack(side=BOTTOM, fill=X)
